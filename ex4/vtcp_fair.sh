...
key1=$1; shift
bwecnhost=$1; shift
bwnonecnhost=$1; shift
bwnet=$1; shift
cutoff=$1; shift
vtcp=$1; shift
label="$1-${bwecnhost}mbps-${bwnonecnhost}mbps-${bwnet}mbps"; shift
red_limit=$1; shift
red_min=$1; shift
red_max=$1; shift
red_avpkt=$1; shift
...
qsize=1000
    ...
    rm -rf $dir
    mkdir -p $dir
    python tcpfair.py --cong reno --cong1 reno --congrest reno --delay $delay -b $bwnet -eB $bwecnhost -neB $bwnonecnhost -d $dir --maxq $qsize -t $time \
    --red_limit $red_limit \
    --red_min $red_min \
    --red_max $red_max \
    --red_avpkt $red_avpkt \
