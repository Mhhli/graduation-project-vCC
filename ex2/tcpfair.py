...
def set_red(iface, red_params):
    ...
    ecn0 = ' '
    print red_params['ecn']
    if red_params['ecn'] == "1":
        ecn0 = 'ecn'
    print ecn0
    cmd = ("tc qdisc change dev %s parent 5:1 handle 6: "
           "red limit %s min %s max %s avpkt %s "
	   "burst %s bandwidth %sMbit probability %s %s" % (iface, red_params['limit'], red_params['min'], red_params['max'], red_params['avpkt'], red_params['burst'], args.bw_net, red_params['prob'],ecn0))
    print cmd
    os.system(cmd)
